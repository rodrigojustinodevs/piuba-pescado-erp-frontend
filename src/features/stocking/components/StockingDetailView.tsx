...
export type StockingDetailViewProps = {
  stocking: Stocking;
  batchLabel: string;
};

export function StockingDetailView({ stocking, batchLabel }: Readonly<StockingDetailViewProps>) {
  const stockingIdLabel = trunc8(stocking.id);
  const totalWeightKg = stocking.quantity * stocking.averageWeight;

  return (
    <div className="-m-4 lg:-m-8 bg-[#F8FAFC] px-8 py-6 min-h-full">
      <p className="text-sm text-slate-600 mb-4">Dashboard / Povoamentos / {stockingIdLabel}</p>
      ...
      <div className="rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex items-start justify-between mb-8 bg-white rounded-t-2xl border border-slate-200 shadow-sm p-8">
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/company/stockings/${stocking.id}/edit`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-8">
          <MetricCard label="Data do Povoamento" value={formatDatePtBR(stocking.stockingDate)} />
          <MetricCard label="Quantidade" value={stocking.quantity} />
          <MetricCard label="Peso médio (kg)" value={formatNumberPtBR(stocking.averageWeight)} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                />
                <InfoCard label="QUANTIDADE" value={stocking.quantity} />
              </div>
            </div>

            <div className="space-y-4">
              <InfoCard label="PESO MÉDIO (KG)" value={formatNumberPtBR(stocking.averageWeight)} />
              <div className="grid grid-cols-2 gap-4">
                <InfoCard label="PESO TOTAL (KG)" value={formatNumberPtBR(totalWeightKg)} />
                <InfoCard label="CRIADO EM" value={formatDatePtBR(stocking.createdAt)} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={`m-8 ${SECTION_CLASS}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InfoCard label="DATA DE CRIAÇÃO" value={formatDatePtBR(stocking.createdAt)} />
            <InfoCard label="ÚLTIMA ATUALIZAÇÃO" value={formatDatePtBR(stocking.updatedAt)} />
          </div>
        </div>
      </div>
    </div>
