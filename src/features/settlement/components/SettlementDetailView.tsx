...
export type SettlementDetailViewProps = {
...
export function SettlementDetailView({
  settlement,
  batchLabel,
}: Readonly<SettlementDetailViewProps>) {
  const settlementIdLabel = trunc8(settlement.id);
  const totalWeightKg = settlement.quantity * settlement.averageWeight;
    ...
    <div className="-m-4 lg:-m-8 bg-[#F8FAFC] px-8 py-6 min-h-full">
      ...
      <div className="rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-8">
          ...
          <MetricCard label="Peso médio (kg)" value={formatNumberPtBR(settlement.averageWeight)} />
